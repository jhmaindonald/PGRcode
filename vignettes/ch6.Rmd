...
document, to give access to the code.  Additionally, settings can be
changed in the immediately following code chunk in the R Markdown (`Rmd`) 
source so that, providing needed R packages are available, it should
'knit' to give text and graphics output (needs better formatting) as 
well as code. 
...
```{r F1_5a, eval=TRUE, message=FALSE, warning=FALSE}
...
```{r F1_5c, eval=TRUE, message=FALSE, warning=FALSE}
auto.arima(LakeHuron)
```

```{r F1_5d, eval=TRUE, message=FALSE, warning=FALSE}
(aaLH0 <- auto.arima(LakeHuron, d=0, approximation=F, stepwise=F))
```

\begin{figure}[h]%F1_5e
```{r 6_4, w=4.0, h=2.4, mgp=c(2,0.5,0), tcl=-0.25, ps=10, top=2.5,echo=FALSE, out.width="48%"}
opar <- par(mar=c(3.1,4.1,2.1,0.6))
y0 <- forecast(aaLH0,  h=12, level=80)
fcast <- forecast(LakeHuron, h=12, level=80)
datall <- cbind(d0=y0[['mean']], ets=fcast[['mean']],
            "ui0"=y0[['upper']],etsUI=fcast[['upper']],
            "li0"=y0[['lower']],etsLI=fcast[['lower']])
...
gph.pred <- xyplot.ts(dat, superpose=T, ylim=ylim, lwd=2, xlab="")
gph.ui <- xyplot.ts(datui, superpose=T, lty=2, lwd=2)
gph.li <- xyplot.ts(datli, superpose=T, lty=2, lwd=2)
update(gph+as.layer(gph.pred)+as.layer(gph.li)+as.layer(gph.ui), xlab="", 
  scales=list(cex=1.0, rot=c(0,90), y=list(tick.number=7)), 
  auto.key=list(columns=2, text=txt, points=F, lines=T, lwd=2, y=1.15),
...
```
\vspace*{-11pt}
\caption[Predictions 12 years ahead, with pointwise CIs -- ARIMA(1,1,2) vs ETS]
{Predictions 12 years ahead, with pointwise confidence 
limits, are compared.  Panel A shows predictions and 80% limits
for the ARMA model (d=0) chosen as `best' overall, and for the 
default exponential time series (ETS) model. Panel B shows the 
graph that results, with 80% and 95% limits, when the ARIMA(1,1,2) 
model selected using {auto.arima()} is supplied as argument 
to the {plot()} function.}{#fig:cfpreds}
\end{figure}

```{r F1_5f}
plot(forecast(aaLH,  h=12, level=c(80,95)))  
plot(forecast(aaLH0,  h=12))  ## `level=c(80,95)` is the default
fcast <- forecast(LakeHuron, h=12)
plot(fcast)
```

```{r F1_5g, eval=TRUE, message=FALSE, warning=FALSE}
auto.arima(LakeHuron, d=0, max.Q=0, approximation=F, stepwise=F)
```

#####                    Use of simulation as a check
\begin{figure}[h]%F1_6b
\vspace*{-7pt}
...
```
\vspace*{-11pt}
\caption[Two simulation runs each for alternative MA3 processes]
{Results of two simulation runs (in different shades of gray), for an MA 
process of order 3, with $b_1$ = $b_2$ = 0, in Panel A with
$b_3$ = 0.25, and in Panel B with $b_3$ = 0.5.  The theoretical 
autocorrelation is shown, in each case, in black.{#fig:simacf}}
\end{figure}
