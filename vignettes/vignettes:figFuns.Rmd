...
vignette: >
  %\VignetteIndexEntry{SelectedFigures-Code}
...
Code for Exercise 31 in Chapter 1 is:
...
gphA2 <- lattice::xyplot(log(bfInterval)~log(n), groups=factor(p), lty=2,
  ...
  panel=function(x,y,...){
    ...
    panel.grid(h=-1,v=-1)
    panel.text(rep(log(max(nval*0.95)),3), plabpos, 
...
```
An alternative way to do the calculations is:

```{r A10_31e, eval=FALSE}
doBF <- function(pval=c(0.05,0.01,0.002), nval=c(4,6,10,20,40,80,160)){
