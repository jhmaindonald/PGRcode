...
'knit' to give text and graphics output as well as code.  There has been
only very limited attention to adjusting the the layout and formatting 
of graphics output to adjust for the change from the LaTeX output used 
in the text to R Markdown.

```{r CodeControl, echo=TRUE}
## xtras=FALSE    ## Set to TRUE to execute code 'extras'
xtras <- F
library(knitr)
...
t2BF <- function(p, N, rscale="medium"){t <- qt(p/2, df=N-1, lower.tail=FALSE)
              BayesFactor::ttest.tstat(t=t, n1=N, rscale=rscale, simple=TRUE)}
nullInt <- c(-.1,.1)
nulltxt <- "null = (-0.1,0.1)"
t2BFint <- function(p,N, rscale="medium"){t <- qt(p/2, df=N-1, lower.tail=FALSE)
  null0 <- BayesFactor::ttest.tstat(t=t, n1=N, nullInterval=nullInt, rscale=rscale,
                                    simple=TRUE)
  alt0 <- BayesFactor::ttest.tstat(t=t, n1=N, nullInterval=nullInt,
  complement=TRUE, rscale=rscale, simple=TRUE)
  alt0/null0}
BFval <- bfIntVal <- matrix(nrow=np,ncol=nlen)
for(i in 1:np)for(j in 1:nlen){
              BFval[i,j] <- t2BF(pval[i], Nval[j], rscale="medium")
              bfIntVal[i,j] <- t2BFint(pval[i], Nval[j], rscale="medium")}
bfDF <- data.frame(n=rep(Nval,rep(np,nlen)), bf=c(BFval), 
  bfInt=c(bfIntVal), t = c(outer(pval/2, Nval-1, qt, lower.tail=FALSE)),
  gp=rep(c("bf05", "bf01", "bf001"),nlen))   
bfDF[['eff']] = bfDF[["t"]]/sqrt(bfDF[['n']])
plabpos <- apply(bfDF[nlen*3-2:0,2:3],1,function(x)mean(log(x))-diff(log(x)*0.1))
gphA1 <- lattice::xyplot(log(bf)~log(n), groups=gp, data=bfDF,
                        panel=function(x,y,...){
...
keyA <- list(corner=c(0.99,0.98), lines=list(col=c(1,1), lty=1:2),
             text=list(c('Point null at 0', nulltxt)))
ylim2 <- log(c(min(bfDF[['bfInt']])-0.05,150))  ## Leaves out two extreme values
gphA2 <- lattice::xyplot(log(bfInt)~log(n), groups=gp, lty=2,
  xlim=c(log(3.5),log(max(Nval)*3.25)), ylim=ylim2, data=bfDF,
  panel=function(x,y,...){
    panel.xyplot(x,y,type='b',...)
    panel.grid(h=-1,v=-1)
    panel.text(rep(log(max(Nval*1.25)),3), plabpos, 
      labels=c('p=0.05','0.01','0.001'), offset=c(-0.5,0.5,0.5),pos=4)
  },
...
gphB <- xyplot(log(eff)~log(n), groups=gp, data=bfDF, pch=1:3, lty=1:3, 
